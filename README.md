...
- **관리자 로그인**: 최초 접속 시 `/login`에서 관리자 번호 입력(httpOnly 쿠키, 7일). **테스트 기본 번호는 `0000`**이며, `ADMIN_ACCESS_CODE` 미설정 시에도 동일하게 동작합니다. 운영 전 반드시 `.env`에서 변경하세요.
...
- **AI 리포트**(선택): `OPENAI_API_KEY` 설정 시 기간 기반 요약
...
브라우저에서 [http://localhost:3000](http://localhost:3000) 을 열면 `/login`으로 이동합니다. 관리자 번호 **테스트: `0000`** 입력 후 대시보드로 들어갑니다.
...
| `ADMIN_ACCESS_CODE` | ⬜ | 관리자 로그인 번호. **미설정 시 기본값 `0000`(테스트용)** — 배포 전 변경 권장 |
...
- **관리자 번호**: 데모용 기본값 `0000`은 공개 저장소·제출용일 뿐입니다. 실서비스에서는 `ADMIN_ACCESS_CODE`를 강하게 설정하고, 필요 시 IP 제한·OAuth 등을 추가하세요.
