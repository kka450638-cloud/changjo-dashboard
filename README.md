...
- **데이터보내기**: 기간·시/도·검색어 반영 **CSV(UTF-8 BOM)** + **엑셀 붙여넣기용 HTML 표** + (선택) **Google 스프레드시트에 행 추가** (`GOOGLE_SHEETS_*`·서비스 계정 JSON 설정 시)
...
| `GOOGLE_SHEETS_SPREADSHEET_ID` | ⬜ | Sheets에 추가: URL `/d/` 뒤 스프레드시트 ID |
| `GOOGLE_SHEETS_TAB` | ⬜ | 붙일 탭 이름 (기본 `Sheet1`) |
| `GOOGLE_SERVICE_ACCOUNT_JSON` | ⬜¹ | 서비스 계정 키 JSON **한 줄** (또는 아래 BASE64) |
| `GOOGLE_SERVICE_ACCOUNT_JSON_BASE64` | ⬜¹ | JSON 파일 전체를 Base64로 넣을 때 (배포 환경에 유리) |

¹ `GOOGLE_SERVICE_ACCOUNT_JSON` 또는 `GOOGLE_SERVICE_ACCOUNT_JSON_BASE64` 중 하나. **스프레드시트를 해당 서비스 계정 이메일(`…@….iam.gserviceaccount.com`)에 편집자로 공유**해야 쓰기가 됩니다.
